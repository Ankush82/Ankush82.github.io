...
The fix is the standard KV cache: compute K and V for each token once, store them, and reuse them on every subsequent step. This turns an O(n²) recomputation pattern into <strong style="color:#1E47E6">O(n) incremental work</strong>.
